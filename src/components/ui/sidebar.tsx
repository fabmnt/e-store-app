...
const DAYS = 7;
const SIDEBAR_COOKIE_MAX_AGE = 60 * 60 * 24 * DAYS;
...
function SidebarProvider({
  ...
  const setOpen = React.useCallback(
    (openedValue: boolean | ((value: boolean) => boolean)) => {
      const openState =
        typeof openedValue === 'function' ? openedValue(open) : openedValue;
      if (setOpenProp) {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: sidebar state
...
function SidebarMenuSkeleton({
  ...
  const width = React.useMemo(() => {
    return `${Math.floor(Math.random() * MIN_WIDTH) + MAX_WIDTH}%`;
  }, []);
